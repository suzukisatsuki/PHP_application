...
SELECT * FROM `countries` WHERE `code` = 'ALB' OR `code` = 'DZA' AND `indep_year` < 1990
...
SELECT MIN(surface_area) FROM countries WHERE continent = 'Asia' GROUP BY continent 
SELECT MIN(surface_area) AS 'アジアの最小表面積' FROM countries GROUP BY continent HAVING continent = 'Asia'
-- 問24
-- アジア大陸の表面積の合計を表示してください。
SELECT SUM(surface_area) AS 'アジア大陸の表面積の合計' FROM countries GROUP BY continent HAVING continent = 'Asia' 
-- 問25
-- 全ての国と言語を表示してください。
SELECT countries.name,countrylanguages.language FROM countrylanguages LEFT JOIN countries ON countrylanguages.country_code = countries.code 

-- 問26
-- 全ての国と言語と市区町村を表示してください。
SELECT countries.name AS '国名',cities.name AS '市区町村名',countrylanguages.language FROM countrylanguages LEFT JOIN countries ON countrylanguages.country_code = countries.code LEFT JOIN cities ON countrylanguages.country_code = cities.country_code
...
FROM celebrities JOIN countries ON celebrities.country_code = countries.code 
...
SELECT country_code,MAX(ce.age),MIN(ce.age) FROM celebrities ce GROUP BY country_code HAVING MAX(ce.age) >= 50 AND MIN(ce.age) <=30 AND country_code IS NOT NULL

-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。
SELECT '1991' AS '誕生年', COUNT(id) FROM celebrities WHERE SUBSTRING(birth, 1, 4) = '1991'
UNION
SELECT '1981', COUNT(id) FROM celebrities WHERE SUBSTRING(birth, 1, 4) = '1981'

-- 問32
-- 有名人の出身国の平均年齢を高い方から順に表示してください。ただし、FROM句はcountriesテーブルとしてください。
SELECT countries.name AS '国名',AVG(age) AS '平均年齢' FROM countries JOIN celebrities ON countries.code = celebrities.country_code GROUP BY countries.name ORDER BY AVG(age) DESC
